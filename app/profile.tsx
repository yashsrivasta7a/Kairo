...
const GlassCard = ({ children, className: cn }: { children: React.ReactNode; className?: string }) => {
    ...
    >
      <SafeAreaView className="flex-1">
        <View className="flex-row items-center justify-between px-5 py-3">
          ...
          <Text style={{ color: dk ? "white" : "#3b0764" }} className="flex-1 text-center text-lg font-bold">
            Profile
          </Text>
          <TouchableOpacity
        ...
        >
          <View className="mb-8 mt-4 items-center">
            ...
            <Text style={{ color: dk ? "#9ca3af" : "#6b7280" }} className="mt-1 text-sm">
              {email}
            </Text>
            {createdAt && <Text className="mt-1 text-xs text-gray-600">Member since {createdAt}</Text>}
          </View>

          {error && (
            <View className="mb-3 rounded-xl px-4 py-2" style={{ backgroundColor: "rgba(239,68,68,0.15)" }}>
              <Text className="text-sm text-red-400">{error}</Text>
            </View>
          )}
          {successMsg && (
            <View className="mb-3 rounded-xl px-4 py-2" style={{ backgroundColor: "rgba(34,197,94,0.15)" }}>
              <Text className="text-sm text-green-400">{successMsg}</Text>
            </View>
          )}

          <Text className="mb-2 ml-1 text-xs font-semibold uppercase tracking-widest text-gray-500">Personal Info</Text>
          <GlassCard>
            <View className="px-4 pb-2 pt-4">
              <Text style={{ color: dk ? "#9ca3af" : "#6b7280" }} className="mb-1 text-xs font-medium">
                Name
              </Text>
              <TextInput
                style={{
          ...
          <Text className="mb-2 ml-1 mt-6 text-xs font-semibold uppercase tracking-widest text-gray-500">Account</Text>
          <GlassCard>
          ...
          <Text className="mb-2 ml-1 mt-6 text-xs font-semibold uppercase tracking-widest text-gray-500">Actions</Text>
          <GlassCard>
            <View className="px-4 py-2">
