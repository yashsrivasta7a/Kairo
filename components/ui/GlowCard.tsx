  ...
  return (
    <Wrap {...wrapProps} style={[{ borderRadius: 20, overflow: 'hidden' }, style]}>
      <Animated.View style={{ flex: 1, transform: [{ scale }], shadowColor: glowColor, shadowOffset: { width: 0, height: 4 }, shadowOpacity: glowOpacity, shadowRadius: 16, elevation: 6 }}>
        <BlurView intensity={30} tint="light" style={{ flex: 1, borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(124,58,237,0.13)' }}>
          <View style={{ flex: 1, backgroundColor: innerBg, padding: noPad ? 0 : pad }}>{children}</View>
        </BlurView>
      </Animated.View>
    </Wrap>
  )
